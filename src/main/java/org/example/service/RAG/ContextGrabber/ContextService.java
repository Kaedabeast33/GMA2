...
public class ContextService {
    ...
    public List<String> buildSelectString(ContextObj contextObj) {
...
// the three types of context matches: by name, by tags, by description
        ...
        for (ContextColumn contextColumn : nameColumn) {
            ...
            // in `src/main/java/com.chipr/ContextGrabber/ContextService.java`
...
//                SELECT
//                    ma.ma_name
//                FROM gma_resources.gma_configs gc
//                JOIN JSON_TABLE(
//                    gc.config_json,
//                    '$.ma[*]'
//                    COLUMNS (
...
//        }
    ...
    public List<InputJson> getQueryResults(List<String> qs) {


        List<InputJson> inputs = new ArrayList<>();

        for(String q : qs){
            System.out.println("Executing query:\n"+q);
            QueryResult qr =QueryResult.getQueryResultObj(q,entityManager);
            for(int i =0;i<qr.getResultSize();i++){
                InputJson input = new InputJson();
                 // old unsafe assignments replaced with null-safe conversions
                 Object col1 = qr.safeGetRow("col1", i);
                 Integer clientId = null;
                 if (col1 != null) {
                     if (col1 instanceof Number) {
                         clientId = ((Number) col1).intValue();
                     } else {
                         try {
                             clientId = Integer.valueOf(col1.toString().trim());
                         } catch (Exception e) {
                 ...
                 Object col2 = qr.safeGetRow("col2", i);
                System.out.println(col2);
                 input.setReportedAt(col2 == null ? null : col2.toString());

                 Object col3 = qr.safeGetRow("col3", i);
                 input.setUploadGroup(col3 == null ? null : col3.toString());

                 Object col4 = qr.safeGetRow("col4", i);
                 input.setUploadGroupDescription(col4 == null ? null : col4.toString());

                 Object col5 = qr.safeGetRow("col5", i);
                 input.setGroupName(col5 == null ? null : col5.toString());

                 Object col6 = qr.safeGetRow("col6", i);
                 input.setGroupNameDescription(col6 == null ? null : col6.toString());

                 Object col7 = qr.safeGetRow("col7", i);
                 input.setTypeName(col7 == null ? null : col7.toString());

                 Object col8 = qr.safeGetRow("col8", i);
                 input.setTypeNameDescription(col8 == null ? null : col8.toString());

                 Object col9 = qr.safeGetRow("col9", i);
                 input.setInputName(col9 == null ? null : col9.toString());

                 Object col10 = qr.safeGetRow("col10", i);
                 input.setInputNameDescription(col10 == null ? null : col10.toString());
                 inputs.add(input);
    ...
    public String getQueryResultsRaw(List<String> qs) {
        List<String> results = new ArrayList<>();
        for(String q : qs){
            System.out.println("Executing query:\n"+q);
            QueryResult qr =QueryResult.getQueryResultObj(q,entityManager);
            results.add(qr.toString());
        }
