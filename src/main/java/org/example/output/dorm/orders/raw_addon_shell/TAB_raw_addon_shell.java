package org.example.output.dorm.orders.raw_addon_shell;
...
import org.example.output.dorm.orders.raw_addon_shell.columns.COL_db_id;
import org.example.output.dorm.orders.raw_addon_shell.columns.COL_account_number;
import org.example.output.dorm.orders.raw_addon_shell.columns.COL_addon;
import org.example.output.dorm.orders.raw_addon_shell.columns.COL_carrier_system;
import org.example.output.dorm.orders.raw_addon_shell.columns.COL_date_db_changed;
import org.example.output.dorm.orders.raw_addon_shell.columns.COL_mobile_number;
import org.example.output.dorm.orders.raw_addon_shell.columns.COL_order_id;
import org.example.output.dorm.orders.raw_addon_shell.columns.COL_order_number;
import org.example.output.dorm.orders.raw_addon_shell.columns.COL_record_id;

import java.util.List;

import org.example.ContextInstance.KDBContext;

public class TAB_raw_addon_shell extends TableTemplate {

    public TAB_raw_addon_shell() {
        super(
            "raw_addon_shell"
,
            "Table storing raw Shell addon records"
,
            new String[]{"orders", "addon", "shell"},
            "tab6def5372-d100-4779-8abf-bad31f4fc3c4"
,
            "dorm"
,
            "orders"
 ...
 KDBContext context = KDBContext.KDB_CONTEXT;
    ...
    private final ColumnTemplate COL_record_id = new COL_record_id();

    public ColumnTemplate getCOL_record_id() {
        return COL_record_id;
    }
...
public String getValues()  {
    return String.format("('%s','%s','%s','%s','%s','%s','%s','%s','%s')\n",
          ...
          getCOL_record_id().getEntityValue().getValue().toString()
    );
}
            @Override
