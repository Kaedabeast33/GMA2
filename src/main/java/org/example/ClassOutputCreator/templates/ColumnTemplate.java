...
//        }
    ...
    public void setEmbedding(List<ColumnTemplate> cols) throws Exception {
        System.out.println("Setting embedding for column: " + this.name);
        if(!this.isEmbedding){
            throw new IllegalStateException("Column is not set as embedding column: " + this.name);
        }
        Double[] embedding;
        try{
            ...
            for(ColumnTemplate col : cols){
                ...
                sb.append(col.getEntityValue().getValue().toString()).append(" \n");
            }
            String embeddingStr = sb.toString();

             embedding = PythonService.makeEmbedding(embeddingStr);

        } catch (Exception e){
            throw new RuntimeException("Failed to get embedding for column: " + this.name, e);
        }
            setEntityValue(embedding);
    }
