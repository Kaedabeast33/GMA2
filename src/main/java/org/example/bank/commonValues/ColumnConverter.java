...
public class ColumnConverter {
    ...
    public static List<KdbAirColumnPersona> toPersonaAirJson(List<AirColumnJson> columnJsonList){
        // Implementation to convert ColumnJson list to Persona JSON format
        return columnJsonList.stream().map(c->(KdbAirColumnPersona) c).toList();

    }

    public static List<KdbAirColumnPersona> toPersonaAirTemplate(List<AirColumnTemplate> columnTemplateList){
        // Implementation to convert ColumnTemplate list to Persona Template format
        return columnTemplateList.stream().map(c->(KdbAirColumnPersona) c).toList();
