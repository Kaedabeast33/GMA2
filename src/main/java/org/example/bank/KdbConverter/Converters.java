...
public class Converters {
    ...
    public static final KdbConverter<Object, Double[]> VECTOR = input -> {
        ...
        inputStr = inputStr.replaceAll("[\\[\\]]", ""); // Remove brackets if present
        String[] parts = inputStr.split(",");
        Double[] result = new Double[parts.length];
        for (int i = 0; i < parts.length; i++) {
            result[i] = Double.parseDouble(parts[i].trim());
        }
        return new EntityValue<>(result, Double[].class);
    };
    ...
    public static final KdbConverter<Object, Boolean> TRUEBOOLEAN = input -> {

        if (input == null || Objects.equals(input.toString(), "")) {
            return new EntityValue<>(null, Boolean.class);
        }

        if (input instanceof Boolean) {
            return new EntityValue<>((Boolean) input, Boolean.class);
        }
        ...
        return new EntityValue<>(isTrue, Boolean.class);
    };
...
//        System.out.println("Converting to TIMESTAMP: " + input);
    ...
    public static final KdbConverter<Object, LocalDateTime> DATETIME = input -> {
        ...
        if (input instanceof Timestamp) return new EntityValue<>(((Timestamp) input).toLocalDateTime(), LocalDateTime.class);

        String s = input.toString().trim();
        if (s.isEmpty()) return new EntityValue<>(null, LocalDateTime.class);

        // First try the shared flexible parser
        try {
            return new EntityValue<>(parseFlexible(s), LocalDateTime.class);
        } catch (Exception ignored) {}

        // Try a few explicit patterns
        ...
        for (DateTimeFormatter fmt : fmts) {
            try {
                return new EntityValue<>(LocalDateTime.parse(s, fmt), LocalDateTime.class);
            } catch (java.time.format.DateTimeParseException ignored) { }
        }

        // Final attempt: normalize 'T' to space and parse
        try {
            String normalized = s.replace('T', ' ');
            return new EntityValue<>(LocalDateTime.parse(normalized, DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")), LocalDateTime.class);
        } catch (java.time.format.DateTimeParseException ex) {
            throw new IllegalArgumentException("Unable to parse LocalDateTime: " + s, ex);
        }
    };
