...
public class TableJson {
    ...
    public TableJson(Identifier identifier,TableComb table,String type){
        if(!type.equalsIgnoreCase("airtable")) {
    ...
    public TableJson(Identifier identifier,TableComb table) throws InvocationTargetException, IllegalAccessException {
        ...
        for (int i = 0; i < fields.size(); i++) {
            ...
            KdbEmbedding kdbEmbedding = field.getKdbEmbedding();
            Class<?> fieldType = field.getFieldType();


            if (kdbColumn != null) {
//                set each annotated Fields into the Columns Json[]
                columns[i] = new ColumnJson(new Identifier(this.identifier),kdbColumn, kdbPrimaryKey, kdbIndex, kdbReference,kdbEmbedding, fieldType);
...
//                        }
        ...
        for (Map.Entry<String, List<ColumnJson>> entry : groupColumnMap.entrySet()) {
            this.columnGroups[k++] = ColumnGroupJson.buildColumnGroupJson(entry);
